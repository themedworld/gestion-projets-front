...
import {
  ...
  LETTER_WIDTH_PX,
  LETTER_WIDTH_PT,
} from "@/condidat/resume/lib/constants";
...
const getIframeInitialContent = (isA4: boolean) => {
  ...
  const allFontFamiliesPreloadLinks = allFontFamilies
    .map(
      (
        font
      ) => `<link rel="preload" as="font" href="public/fonts/${font}-Regular.ttf" type="font/ttf" crossorigin="anonymous">
<link rel="preload" as="font" href="public/fonts/${font}-Bold.ttf" type="font/ttf" crossorigin="anonymous">`
    )
    .join("");
  ...
  const allFontFamiliesFontFaces = allFontFamilies
    .map(
      (
        font
      ) => `@font-face {font-family: "${font}"; src: url("public/fonts/${font}-Regular.ttf");}
@font-face {font-family: "${font}"; src: url("public/fonts/${font}-Bold.ttf"); font-weight: bold;}`
    )
    .join("");
